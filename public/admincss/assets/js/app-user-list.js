    ...
    const dt_user = new DataTable(dt_user_table, {
      ...
      columnDefs: [
        ...
        },
        {
          targets: 3,
          render: function (data, type, full, meta) {
            const roles = full.roles.map(role => role.name).join(', ');
            return `<span class='text-truncate d-flex align-items-center text-heading'>${roles}</span>`;
        }
      ...
      initComplete: function () {
        ...
        // Role filter
        createFilter(3, '.user_role', 'User Role', 'Select Role');
